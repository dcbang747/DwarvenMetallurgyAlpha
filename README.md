...
For the bronze armor pieces, the `.bbmodel` files in the repository root are the source files used to export the GeckoLib `.geo.json` files.  
If you only edit `dwarvencraft/src/client/resources/assets/dwarvencraft/geckolib/models/*.geo.json`, those edits can be lost the next time the models are re-exported from Blockbench.
